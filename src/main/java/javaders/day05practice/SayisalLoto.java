...
*/
    ...
    public static void startLoto() {
        ...
        for (int i = 0; i < slots; i++) {
            ...
            do {
                isRepeat=false; //tekrarlanmıyorsa loop calissin.
                rnd1 = rnd.nextInt(max+1-min)+min;

                for (int j = 0; j < i; j++) {
                    if (lotoArr[j]==rnd1){
                        isRepeat=true;
                        break; //dizide tekrarli varsa arr ekleme
                    }
                }//forj
            }while(isRepeat); //tekraralanan bir sayi uretirse tekrar do-while loop'un basina don.
